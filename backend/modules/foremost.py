...
class Foremost(Module):
    ...
    def run(self):
        self.set_config_status("foremost", "running")
        ...
        if self.config["md5_image"] != self.config["md5_full"] \
           and os.path.isfile(f"{UPLOAD_FOLDER}/{md5_image}/foremost.7z"):
            cmd(f"cp {UPLOAD_FOLDER}/{md5_image}/foremost.7z "
                f"{self.folder}/foremost.7z")
        else:  # Else compute
            image = self.config["image"]
            c_input = f"{self.folder}/{image}"  # image.png
            cmd(f"foremost -i {c_input} -o {self.folder}/foremost 2>&1")
            cmd(
                f"7z a {self.folder}/foremost.7z {self.folder}/foremost/*")
            cmd(f"rm -r {self.folder}/foremost")
        global RUNNING
        RUNNING.remove(self.md5)
        self.set_config_status("foremost", "finished")
